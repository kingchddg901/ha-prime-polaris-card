var y="ha-prime-polaris-card",z="0.2.0",A="grill",D={climate:"climate.{prefix}",chamber_temp:"sensor.{prefix}_chamber_temperature",running_status:"sensor.{prefix}_running_status",active_mode:"sensor.{prefix}_active_mode",active_smoke_level:"sensor.{prefix}_active_smoke_level",last_alarm:"sensor.{prefix}_last_alarm",probe_1_temp:"sensor.{prefix}_probe_1_temperature",probe_2_temp:"sensor.{prefix}_probe_2_temperature",probe_1_eta:"sensor.{prefix}_probe_1_eta",probe_2_eta:"sensor.{prefix}_probe_2_eta",setpoint:"number.{prefix}_temperature",smoke_level:"number.{prefix}_smoke_level",probe_1_target:"number.{prefix}_probe_1_target",probe_2_target:"number.{prefix}_probe_2_target",push_dedupe:"number.{prefix}_push_alert_dedupe",smoke_mode:"switch.{prefix}_smoke_mode",winter_mode:"switch.{prefix}_winter_mode",alarm:"switch.{prefix}_temperature_alarm",cook_session:"switch.{prefix}_cook_session",push_alerts:"switch.{prefix}_push_alerts",notes:"text.{prefix}_cook_notes",protein:"text.{prefix}_cook_protein",weight_lb:"text.{prefix}_cook_weight_lb",ambient_override:"text.{prefix}_cook_ambient_override",wind_override:"text.{prefix}_cook_wind_override"},E=4,O=3e4;var ne=/^[a-z_]+\.[a-z0-9_]+$/;function N(t,e,s){let o={value:null,source:""};if(!t||!e)return o;if(ne.test(e)){let a=t.states[e];if(!a)return{value:null,source:`${e} (missing)`};if(e.startsWith("weather.")&&a.attributes?.[s]!=null){let p=parseFloat(a.attributes[s]);return Number.isFinite(p)?{value:p,source:e}:o}let n=parseFloat(a.state);return Number.isFinite(n)?{value:n,source:e}:o}let r=parseFloat(e);return Number.isFinite(r)?{value:r,source:"literal"}:o}function I(t,e){if(!t)return null;let s=l=>D[l].replace("{prefix}",e),o=l=>t.states[s(l)]??null,r=l=>{let c=o(l);if(!c)return null;let d=parseFloat(c.state);return Number.isFinite(d)?d:null},a=l=>o(l)?.state==="on",n=l=>{let c=o(l);return c?c.state==="unknown"||c.state==="unavailable"?"":c.state:""},p=l=>{let c=o(l===1?"probe_1_eta":"probe_2_eta");if(!c)return{minutes:null,in_stall:!1,stdev:null,source:null};let d=parseFloat(c.state);return{minutes:Number.isFinite(d)?d:null,in_stall:!!c.attributes?.in_stall,stdev:c.attributes?.stall_stdev??null,source:c.attributes?.prior_source??null,samples:c.attributes?.samples??0}},u=o("climate"),m=r("setpoint")??u?.attributes?.temperature??null;return{prefix:e,entityIds:Object.fromEntries(Object.keys(D).map(l=>[l,s(l)])),chamber:r("chamber_temp")??u?.attributes?.current_temperature??null,setpoint:m,chamberDelta:r("chamber_temp")!=null&&m!=null?r("chamber_temp")-m:null,runningStatus:n("running_status")||null,activeMode:n("active_mode")||"off",smokeLevel:r("active_smoke_level"),smokeOn:a("smoke_mode"),winterOn:a("winter_mode"),alarmOn:a("alarm"),probe1:{temp:r("probe_1_temp"),target:r("probe_1_target"),...p(1)},probe2:{temp:r("probe_2_temp"),target:r("probe_2_target"),...p(2)},cookSession:a("cook_session"),notes:n("notes"),protein:n("protein"),weight_lb:n("weight_lb"),ambient:n("ambient_override"),wind:n("wind_override"),ambientResolved:N(t,n("ambient_override"),"temperature"),windResolved:N(t,n("wind_override"),"wind_speed"),pushOn:a("push_alerts"),pushDedupe:r("push_dedupe"),account:(()=>{let l=o("climate"),c=l?.attributes?.email??null,d=l?.attributes?.token_expiry??null,_=d?Math.max(0,Math.round((d*1e3-Date.now())/864e5)):null;return{email:c,daysToExpiry:_}})(),lastAlarm:(()=>{let l=o("last_alarm");return!l||l.state==="unknown"||l.state==="unavailable"||!l.state?null:{title:l.state,body:l.attributes?.body??"",captured_at:l.attributes?.captured_at??null,source:l.attributes?.source??null}})()}}function F(t,e){if(!t||!e)return{};let s=(o,r,a={})=>t.callService(o,r,a);return{setSetpoint:o=>s("number","set_value",{entity_id:e.entityIds.setpoint,value:o}),setSmokeLevel:o=>s("number","set_value",{entity_id:e.entityIds.smoke_level,value:o}),setProbeTarget:(o,r)=>s("number","set_value",{entity_id:e.entityIds[`probe_${o}_target`],value:r}),toggle:o=>{let r=e.entityIds[o],a=t.states[r]?.state==="on";return s("switch",a?"turn_off":"turn_on",{entity_id:r})},setText:(o,r)=>s("text","set_value",{entity_id:e.entityIds[o],value:r??""}),powerOff:()=>s("climate","set_hvac_mode",{entity_id:e.entityIds.climate,hvac_mode:"off"})}}var i={bg:"#0d1117",panel:"#161b22",panelBorder:"#30363d",text:"#e6edf3",textDim:"#8b949e",accent:"#fb923c",accentDim:"#9a3412",cool:"#3b82f6",hot:"#ef4444",steady:"#22c55e",smoke:"#a78bfa",alarm:"#f87171",stall:"#f59e0b",probe1:"#ef4444",probe2:"#06b6d4",chartChamber:"#fb923c",chartProbe1:"#ef4444",chartProbe2:"#06b6d4"},P=5;var B=`
  :host {
    display: block;
    color: ${i.text};
...
`;var Y="apexcharts-card",T=class{constructor(e){this._host=e,this._mode=null,this._apexEl=null,this._canvas=null,this._series={},this._historyLoaded=!1,this._lastConfig=null}async update(e,s){let o=s.join("|");if(o!==this._lastConfig&&(this._lastConfig=o,this._historyLoaded=!1,this._series={},this._host.innerHTML="",this._mode=customElements.get(Y)?"apex":"canvas",this._mode==="apex"?this._mountApex(s):this._mountCanvas()),this._mode==="apex"){this._apexEl.hass=e;return}this._historyLoaded||(this._historyLoaded=!0,await this._loadHistory(e,s)),this._appendLive(e,s),this._draw(s)}_mountApex(e){let s=e.map((r,a)=>({entity:r,name:this._labelFor(r),yaxis_id:"temp",color:[i.chartChamber,i.chartProbe1,i.chartProbe2][a]||"#fff",type:a===0?"area":"line",opacity:a===0?.25:1,stroke_width:2})),o=document.createElement(Y);o.setConfig({header:{show:!1},graph_span:`${E}h`,update_interval:`${O/1e3}s`,yaxis:[{id:"temp",decimals:0,apex_config:{title:{text:"\xB0F"},forceNiceScale:!0}}],series:s}),this._apexEl=o,this._host.appendChild(o)}_mountCanvas(){this._canvas=document.createElement("canvas"),this._host.appendChild(this._canvas)}async _loadHistory(e,s){let a=`history/period/${new Date(Date.now()-E*3600*1e3).toISOString()}?filter_entity_id=${s.join(",")}&minimal_response`;try{let n=await e.callApi("GET",a);for(let p=0;p<s.length;p++){let u=s[p],m=(n?.[p]??[]).map(l=>[Date.parse(l.last_changed),parseFloat(l.state)]).filter(([l,c])=>Number.isFinite(l)&&Number.isFinite(c));this._series[u]=m}}catch(n){console.warn("[ha-prime-polaris-card] history fetch failed:",n)}}_appendLive(e,s){let o=Date.now(),r=o-E*3600*1e3;for(let a of s){let n=this._series[a]??(this._series[a]=[]),p=parseFloat(e.states[a]?.state);if(Number.isFinite(p)){let u=n.length?n[n.length-1][0]:0;o-u>=5e3&&n.push([o,p])}for(;n.length&&n[0][0]<r;)n.shift()}}_draw(e){let s=this._canvas;if(!s)return;let o=window.devicePixelRatio||1,r=s.clientWidth||600,a=240;s.width=r*o,s.height=a*o;let n=s.getContext("2d");n.scale(o,o),n.clearRect(0,0,r,a);let p=e.flatMap(h=>this._series[h]??[]);if(p.length<2){n.fillStyle=i.textDim,n.font="12px system-ui",n.fillText("collecting data\u2026",16,24);return}let u=p.map(h=>h[0]),m=p.map(h=>h[1]),l=Math.min(...u),c=Math.max(...u),d=Math.min(...m)-5,_=Math.max(...m)+5,b={l:36,r:12,t:12,b:22},g=h=>b.l+(h-l)/Math.max(1,c-l)*(r-b.l-b.r),v=h=>b.t+(1-(h-d)/Math.max(1,_-d))*(a-b.t-b.b);n.strokeStyle=i.panelBorder,n.fillStyle=i.textDim,n.font="11px system-ui",n.lineWidth=1;let k=4;for(let h=0;h<=k;h++){let w=d+h/k*(_-d),f=v(w);n.beginPath(),n.moveTo(b.l,f),n.lineTo(r-b.r,f),n.stroke(),n.fillText(w.toFixed(0),6,f+4)}let S=[i.chartChamber,i.chartProbe1,i.chartProbe2];e.forEach((h,w)=>{let f=this._series[h]??[];if(!(f.length<2)){if(n.strokeStyle=S[w]||"#fff",n.lineWidth=w===0?1.5:2,w===0){n.fillStyle=S[0]+"40",n.beginPath(),n.moveTo(g(f[0][0]),v(d));for(let[M,C]of f)n.lineTo(g(M),v(C));n.lineTo(g(f[f.length-1][0]),v(d)),n.closePath(),n.fill()}n.beginPath(),n.moveTo(g(f[0][0]),v(f[0][1]));for(let[M,C]of f)n.lineTo(g(M),v(C));n.stroke()}})}_labelFor(e){return e.includes("chamber")?"Chamber":e.includes("probe_1")?"Probe 1":e.includes("probe_2")?"Probe 2":e}};var $=-135,R=135,q=R-$;function X(t){let e=t.size??220,s=e/2,o=e/2,r=e/2-18,a=t.min,n=t.max,p=t.current??a,u=t.target??a,m=V(p,a,n),l=V(u,a,n),c=t.fillColor??le(p,u),d=G(s,o,r,$,R),_=G(s,o,r,$,m),b=L(s,o,r,l);return`
    <svg
      viewBox="0 0 ${e} ${e}"
      class="arc-gauge"
      data-gauge="true"
  ...
  `}function V(t,e,s){let r=(Math.max(e,Math.min(s,t))-e)/(s-e);return $+r*q}function ae(t,e,s){let o=t;for(;o<$-180;)o+=360;for(;o>R+180;)o-=360;let a=(Math.max($,Math.min(R,o))-$)/q;return Math.round(e+a*(s-e))}function L(t,e,s,o){let r=(o-90)*Math.PI/180;return{x:t+s*Math.cos(r),y:e+s*Math.sin(r)}}function G(t,e,s,o,r){let a=L(t,e,s,o),n=L(t,e,s,r),p=Math.abs(r-o)>180?1:0;return`M ${a.x} ${a.y} A ${s} ${s} 0 ${p} 1 ${n.x} ${n.y}`}function le(t,e){if(t==null||e==null)return i.accent;let s=t-e;return s>P?i.hot:s<-P?i.cool:i.steady}function W(t,e){if(!t)return;let s=parseFloat(t.dataset.min),o=parseFloat(t.dataset.max),r=parseFloat(t.dataset.cx),a=parseFloat(t.dataset.cy),n=!1,p=null,u=(c,d)=>{let _=t.getBoundingClientRect(),b=t.viewBox.baseVal.width/_.width,g=t.viewBox.baseVal.height/_.height,v=(c-_.left)*b-r,k=(d-_.top)*g-a,S=Math.atan2(k,v)*180/Math.PI+90;return ae(S,s,o)},m=c=>{if(!n)return;p=u(c.clientX,c.clientY)},l=()=>{n&&(n=!1,document.removeEventListener("pointermove",m),document.removeEventListener("pointerup",l),p!=null&&e(p))};t.addEventListener("pointerdown",c=>{let d=c.target,_=d.dataset?.thumb==="true";(d.tagName==="path"||_)&&(c.preventDefault(),n=!0,p=u(c.clientX,c.clientY),document.addEventListener("pointermove",m),document.addEventListener("pointerup",l))})}function x(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function j(t){if(!t)return"";let{chamber:e,setpoint:s,chamberDelta:o}=t,r=o!=null?`\u0394 ${o>0?"+":""}${o.toFixed(0)}\xB0F`:"";return`
  ...
  `}function se(t){if(!t)return"";let e=t.lastAlarm;return e?`
    <div class="alarm-banner">
      <span><strong>${x(e.title)}</strong> \xB7 ${x(e.body)}</span>
      <span class="small">${x(e.source??"")}</span>
    </div>
  ...
  `}var H=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._config=null,this._state=null,this._chart=null,this._view="live",this._renderQueued=!1}setConfig(e){this._config={entity_prefix:e?.entity_prefix??A,title:e?.title??null,ambient_entity:e?.ambient_entity??null,...e},this._lastSig=null}set hass(e){this._hass=e,this._config&&(this._state=I(e,this._config.entity_prefix),this._scheduleRender())}getCardSize(){return 8}static getConfigElement(){return null}static getStubConfig(){return{entity_prefix:A}}_scheduleRender(){this._renderQueued||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this._render()}))}_render(){let e=this._state;(!this.shadowRoot.firstElementChild||this.shadowRoot.firstElementChild.dataset.view!==this._view)&&(this._stampShell(),this._wireEvents()),this._fill("tabs",te(this._view)),this._view==="live"?this._renderLive(e):this._renderSetup(e)}_stampShell(){let e=`
    ...
    `,this._view!=="live"&&(this._chart=null)}_renderLive(e){this._fill("alarm",se(e)),this._fill("chips",Q(e)),this._fill("chamber",j(e)),this._fill("cookHeader",J(e)),this._fill("probes",K(e)),this._fillPreserveFocus("controls",Z(e)),this._fillPreserveFocus("session",ee(e));let s=this.shadowRoot.querySelector('[data-gauge="true"]');if(s){let o=F(this._hass,this._state);W(s,r=>o.setSetpoint(r))}if(this._hass&&e){if(!this._chart){let r=this.shadowRoot.querySelector('[data-slot="chart"]');this._chart=new T(r)}let o=[e.entityIds.chamber_temp,e.entityIds.probe_1_temp,e.entityIds.probe_2_temp];this._chart.update(this._hass,o)}}_renderSetup(e){this._fillPreserveFocus("setup",re(e,this._config))}_fill(e,s){let o=this.shadowRoot.querySelector(`[data-slot="${e}"]`);o&&(o.innerHTML=s)}_fillPreserveFocus(e,s){let o=this.shadowRoot.querySelector(`[data-slot="${e}"]`);if(!o)return;let r=this.shadowRoot.activeElement;if(!(r&&r.dataset?.input&&o.contains(r))){o.innerHTML=s;return}let n=r.dataset.input,p=r.value,u=r.selectionStart,m=r.selectionEnd;o.innerHTML=s;let l=o.querySelector(`[data-input="${n}"]`);if(l){l.value=p,l.focus();try{l.setSelectionRange(u,m)}catch{}}}_wireEvents(){this.shadowRoot.addEventListener("click",e=>{let s=e.target.closest("[data-action]");s&&this._dispatchAction(s.dataset.action)}),this.shadowRoot.addEventListener("change",e=>{let s=e.target.closest("[data-input]");if(!s)return;let o=F(this._hass,this._state),r=s.dataset.input;if(r==="setpoint"){let n=parseInt(s.value,10);Number.isFinite(n)&&o.setSetpoint(Math.max(180,Math.min(500,n)));return}if(r==="smoke_level"){let n=parseInt(s.value,10);Number.isFinite(n)&&o.setSmokeLevel(Math.max(0,Math.min(10,n)));return}if(r==="push_dedupe"){let n=parseInt(s.value,10);Number.isFinite(n)&&this._hass.callService("number","set_value",{entity_id:this._state.entityIds.push_dedupe,value:Math.max(10,Math.min(3600,n))});return}let a={notes:"notes",protein:"protein",weight_lb:"weight_lb",ambient:"ambient_override",wind:"wind_override"};a[r]&&o.setText(a[r],s.value)}),this.shadowRoot.addEventListener("input",e=>{let s=e.target.closest('[data-input="smoke_level"]');if(!s)return;let o=this.shadowRoot.querySelector('[data-bind="smoke-level-readout"]');o&&(o.textContent=s.value)})}_dispatchAction(e){if(!this._hass||!this._state)return;let s=F(this._hass,this._state),o=this._state.setpoint??225;switch(e){case"toggle-session":s.toggle("cook_session");break;case"toggle-smoke":s.toggle("smoke_mode");break;case"toggle-winter":s.toggle("winter_mode");break;case"toggle-alarm":s.toggle("alarm");break;case"toggle-push":s.toggle("push_alerts");break;case"temp-up":s.setSetpoint(Math.min(500,o+1));break;case"temp-down":s.setSetpoint(Math.max(180,o-1));break;case"power-off":s.powerOff();break;case"set-view-live":this._view="live",this._scheduleRender();break;case"set-view-setup":this._view="setup",this._scheduleRender();break}}};customElements.get(y)||customElements.define(y,H);window.customCards=window.customCards||[];window.customCards.some(t=>t.type===y)||window.customCards.push({type:y,name:"Prime Polaris Grill",description:"Live cook dashboard for the Prime Polaris pellet grill integration.",preview:!0});console.info(`%c ${y} %c v${z} `,"color:#fb923c;background:#0d1117;padding:2px 6px;border-radius:3px 0 0 3px;font-weight:600","color:#0d1117;background:#fb923c;padding:2px 6px;border-radius:0 3px 3px 0");
