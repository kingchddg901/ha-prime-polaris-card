...
`;var W="apexcharts-card",F=class{constructor(e){this._host=e,this._mode=null,this._apexEl=null,this._canvas=null,this._series={},this._historyLoaded=!1,this._lastConfig=null}async update(e,s){let i=s.join("|");if(i!==this._lastConfig&&(this._lastConfig=i,this._historyLoaded=!1,this._series={},this._host.innerHTML="",this._mode=customElements.get(W)?"apex":"canvas",this._mode==="apex"?this._mountApex(s):this._mountCanvas()),this._mode==="apex"){this._apexEl.hass=e;return}this._historyLoaded||(this._historyLoaded=!0,await this._loadHistory(e,s)),this._appendLive(e,s),this._draw(s)}_mountApex(e){let s=e.map((o,a)=>({entity:o,name:this._labelFor(o),yaxis_id:"temp",color:[n.chartChamber,n.chartProbe1,n.chartProbe2][a]||"#fff",type:a===0?"area":"line",opacity:a===0?.25:1,stroke_width:2})),i=document.createElement(W);i.setConfig({header:{show:!1},graph_span:`${k}h`,update_interval:`${L/1e3}s`,yaxis:[{id:"temp",decimals:0,apex_config:{title:{text:"\xB0F"},forceNiceScale:!0}}],series:s}),this._apexEl=i,this._host.appendChild(i)}_mountCanvas(){this._canvas=document.createElement("canvas"),this._host.appendChild(this._canvas)}async _loadHistory(e,s){let a=`history/period/${new Date(Date.now()-k*3600*1e3).toISOString()}?filter_entity_id=${s.join(",")}&minimal_response`;try{let r=await e.callApi("GET",a);for(let c=0;c<s.length;c++){let u=s[c],m=(r?.[c]??[]).map(l=>[Date.parse(l.last_changed),parseFloat(l.state)]).filter(([l,p])=>Number.isFinite(l)&&Number.isFinite(p));this._series[u]=m}}catch(r){console.warn("[ha-prime-polaris-card] history fetch failed:",r)}}_appendLive(e,s){let i=Date.now(),o=i-k*3600*1e3;for(let a of s){let r=this._series[a]??(this._series[a]=[]),c=parseFloat(e.states[a]?.state);if(Number.isFinite(c)){let u=r.length?r[r.length-1][0]:0;i-u>=5e3&&r.push([i,c])}for(;r.length&&r[0][0]<o;)r.shift()}}_draw(e){let s=this._canvas;if(!s)return;let i=window.devicePixelRatio||1,o=s.clientWidth||600,a=240;s.width=o*i,s.height=a*i;let r=s.getContext("2d");r.scale(i,i),r.clearRect(0,0,o,a);let c=e.flatMap(h=>this._series[h]??[]);if(c.length<2){r.fillStyle=n.textDim,r.font="12px system-ui",r.fillText("collecting data\u2026",16,24);return}let u=c.map(h=>h[0]),m=c.map(h=>h[1]),l=Math.min(...u),p=Math.max(...u),d=Math.min(...m)-5,x=Math.max(...m)+5,f={l:36,r:12,t:12,b:22},$=h=>f.l+(h-l)/Math.max(1,p-l)*(o-f.l-f.r),v=h=>f.t+(1-(h-d)/Math.max(1,x-d))*(a-f.t-f.b);r.strokeStyle=n.panelBorder,r.fillStyle=n.textDim,r.font="11px system-ui",r.lineWidth=1;let N=4;for(let h=0;h<=N;h++){let g=d+h/N*(x-d),b=v(g);r.beginPath(),r.moveTo(f.l,b),r.lineTo(o-f.r,b),r.stroke(),r.fillText(g.toFixed(0),6,b+4)}let O=[n.chartChamber,n.chartProbe1,n.chartProbe2];e.forEach((h,g)=>{let b=this._series[h]??[];if(!(b.length<2)){if(r.strokeStyle=O[g]||"#fff",r.lineWidth=g===0?1.5:2,g===0){r.fillStyle=O[0]+"40",r.beginPath(),r.moveTo($(b[0][0]),v(d));for(let[R,T]of b)r.lineTo($(R),v(T));r.lineTo($(b[b.length-1][0]),v(d)),r.closePath(),r.fill()}r.beginPath(),r.moveTo($(b[0][0]),v(b[0][1]));for(let[R,T]of b)r.lineTo($(R),v(T));r.stroke()}})}_labelFor(e){return e.includes("chamber")?"Chamber":e.includes("probe_1")?"Probe 1":e.includes("probe_2")?"Probe 2":e}};var E=-135,Y=135,le=Y-E;function V(t){let e=t.size??220,s=e/2,i=e/2,o=e/2-18,a=t.min,r=t.max,c=t.current??a,u=t.target??a,m=j(c,a,r),l=j(u,a,r),p=t.fillColor??ce(c,u),d=U(s,i,o,E,Y),x=U(s,i,o,E,m),f=D(s,i,o,l);return`
  ...
  `}function G(t){return t?String(t).split(/(\s+)/).map(e=>e.length===0||/\s+/.test(e)?e:e[0].toUpperCase()+e.slice(1).toLowerCase()).join(""):""}function J(t){if(!t)return"";let e=t.cookSession,s=e?t.protein?`${_(t.protein)}${t.weight_lb?" \xB7 "+_(t.weight_lb)+" lb":""}`:"recording":"no active cook",i=t.notes?_(t.notes):e?"":"flip the switch when you start a real cook",o=[];t.ambientResolved.value!=null&&o.push(`ambient <strong>${t.ambientResolved.value.toFixed(0)}\xB0F</strong>`),t.windResolved.value!=null&&o.push(`wind <strong>${t.windResolved.value.toFixed(1)}</strong>`);let a=o.length?`<div class="small env-line">${o.join(" \xB7 ")}</div>`:"";return`
  ...
  `:'<div class="alarm-banner dim">no alarms</div>'}function y(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}var ne=/\b(indoor|inside|bedroom|bathroom|kitchen|living|hallway|office|garage|attic|basement|fridge|freezer|oven|refrigerator|dishwasher|washer|dryer|cpu|gpu|battery|server|chip|water_heater|coolant|pool|spa|jacuzzi|hot_tub|aquarium|fishtank|car|vehicle|engine)\b/i,pe=/\b(outdoor|outside|exterior|patio|deck|porch|backyard|yard|weather|ambient|station|pws|awn|tempest|davis|ecowitt|netatmo)\b/i,oe=/(ambient[a-z]*network|ambientweather|tempest|weatherflow|openweathermap|dark[- ]?sky|met\.no|met office|accuweather|weather\.gov|nws|pirate weather|aemet|wunderground|weatherapi|netatmo|ecowitt|davis\b|wxinsight|meteo|aprs|ambientcwop|cwop|airnow)/i,de=/\b(mph|m\/s|km\/h|knots?)\b/i,ue=/\b(wind|gust)\b/i;function me(t,e){if(!t)return[];let s=e?new RegExp(`^[a-z_]+\\.${e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}_`,"i"):null,i=[];for(let[o,a]of Object.entries(t.states)){if(s&&s.test(o)||ne.test(o))continue;if(o.startsWith("weather.")){let x=parseFloat(a.attributes?.temperature);Number.isFinite(x)&&i.push({eid:o,value:x,unit:"\xB0F"});continue}let r=a.attributes?.device_class,c=a.attributes?.unit_of_measurement||"";if(!(r==="temperature"||/°[FC]|degf|degc/i.test(c)))continue;let m=pe.test(o),l=String(a.attributes?.attribution||""),p=oe.test(l);if(!m&&!p)continue;let d=parseFloat(a.state);Number.isFinite(d)&&i.push({eid:o,value:d,unit:c||"\xB0F"})}return i.sort((o,a)=>{let r=o.eid.startsWith("weather.")?0:1,c=a.eid.startsWith("weather.")?0:1;return r-c||o.eid.localeCompare(a.eid)}).slice(0,6)}function he(t,e){if(!t)return[];let s=e?new RegExp(`^[a-z_]+\\.${e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}_`,"i"):null,i=[];for(let[o,a]of Object.entries(t.states)){if(s&&s.test(o)||ne.test(o))continue;let r=a.attributes?.unit_of_measurement||"";if(!de.test(r))continue;let c=ue.test(o),u=String(a.attributes?.attribution||""),m=oe.test(u);if(!c&&!m)continue;let l=parseFloat(a.state);Number.isFinite(l)&&i.push({eid:o,value:l,unit:r})}return i.sort((o,a)=>o.eid.localeCompare(a.eid)).slice(0,4)}function be(t,e,s){return t.length?`
    <div class="cand-row">
      ${t.map(i=>`
  ...
  `:""}function ie(t,e,s,i,o,a){let r="";return s&&(r=i&&i.value!=null?`<span class="resolved-badge ok">${i.value.toFixed(1)}${o}</span>`:'<span class="resolved-badge bad">unresolved</span>'),`
    ...
    ${r||"<span></span>"}
    <span></span>
    <div class="cand-host">${be(a,e,s)}</div>
    <span></span>
  `}function re(t,e,s){if(!t||!t.account)return`
    ...
    <div class="panel">
      ...
      <div class="session sensor-grid">
        ${ie("Ambient sensor","ambient",t.ambient,t.ambientResolved,"\xB0F",me(s,e?.entity_prefix))}
        ${ie("Wind sensor","wind",t.wind,t.windResolved,"",he(s,e?.entity_prefix))}
      </div>
      <div class="small" style="margin-top:8px;">
