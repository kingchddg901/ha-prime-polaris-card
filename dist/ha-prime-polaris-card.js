var $="ha-prime-polaris-card",z="0.2.3",T="grill",R={climate:"climate.{prefix}",chamber_temp:"sensor.{prefix}_chamber_temperature",running_status:"sensor.{prefix}_running_status",active_mode:"sensor.{prefix}_active_mode",active_smoke_level:"sensor.{prefix}_active_smoke_level",last_alarm:"sensor.{prefix}_last_alarm",probe_1_temp:"sensor.{prefix}_probe_1_temperature",probe_2_temp:"sensor.{prefix}_probe_2_temperature",probe_1_eta:"sensor.{prefix}_probe_1_eta",probe_2_eta:"sensor.{prefix}_probe_2_eta",setpoint:"number.{prefix}_temperature",smoke_level:"number.{prefix}_smoke_level",probe_1_target:"number.{prefix}_probe_1_target",probe_2_target:"number.{prefix}_probe_2_target",push_dedupe:"number.{prefix}_push_alert_dedupe",smoke_mode:"switch.{prefix}_smoke_mode",winter_mode:"switch.{prefix}_winter_mode",alarm:"switch.{prefix}_temperature_alarm",cook_session:"switch.{prefix}_cook_session",push_alerts:"switch.{prefix}_push_alerts",notes:"text.{prefix}_cook_notes",protein:"text.{prefix}_cook_protein",weight_lb:"text.{prefix}_cook_weight_lb",ambient_override:"text.{prefix}_cook_ambient_override",wind_override:"text.{prefix}_cook_wind_override"},y=4,H=3e4;var oe=/^[a-z_]+\.[a-z0-9_]+$/;function O(s,e,t){let o={value:null,source:""};if(!s||!e)return o;if(oe.test(e)){let a=s.states[e];if(!a)return{value:null,source:`${e} (missing)`};if(e.startsWith("weather.")&&a.attributes?.[t]!=null){let c=parseFloat(a.attributes[t]);return Number.isFinite(c)?{value:c,source:e}:o}let r=parseFloat(a.state);return Number.isFinite(r)?{value:r,source:e}:o}let n=parseFloat(e);return Number.isFinite(n)?{value:n,source:"literal"}:o}function N(s,e){if(!s)return null;let t=l=>R[l].replace("{prefix}",e),o=l=>s.states[t(l)]??null,n=l=>{let p=o(l);if(!p)return null;let d=parseFloat(p.state);return Number.isFinite(d)?d:null},a=l=>o(l)?.state==="on",r=l=>{let p=o(l);return p?p.state==="unknown"||p.state==="unavailable"?"":p.state:""},c=l=>{let p=o(l===1?"probe_1_eta":"probe_2_eta");if(!p)return{minutes:null,in_stall:!1,stdev:null,source:null};let d=parseFloat(p.state);return{minutes:Number.isFinite(d)?d:null,in_stall:!!p.attributes?.in_stall,stdev:p.attributes?.stall_stdev??null,source:p.attributes?.prior_source??null,samples:p.attributes?.samples??0}},m=o("climate"),h=n("setpoint")??m?.attributes?.temperature??null;return{prefix:e,entityIds:Object.fromEntries(Object.keys(R).map(l=>[l,t(l)])),chamber:n("chamber_temp")??m?.attributes?.current_temperature??null,setpoint:h,chamberDelta:n("chamber_temp")!=null&&h!=null?n("chamber_temp")-h:null,runningStatus:r("running_status")||null,activeMode:r("active_mode")||"off",smokeLevel:n("active_smoke_level"),smokeOn:a("smoke_mode"),winterOn:a("winter_mode"),alarmOn:a("alarm"),probe1:{temp:n("probe_1_temp"),target:n("probe_1_target"),...c(1)},probe2:{temp:n("probe_2_temp"),target:n("probe_2_target"),...c(2)},cookSession:a("cook_session"),notes:r("notes"),protein:r("protein"),weight_lb:r("weight_lb"),ambient:r("ambient_override"),wind:r("wind_override"),ambientResolved:O(s,r("ambient_override"),"temperature"),windResolved:O(s,r("wind_override"),"wind_speed"),pushOn:a("push_alerts"),pushDedupe:n("push_dedupe"),account:(()=>{let l=o("climate"),p=l?.attributes?.email??null,d=l?.attributes?.token_expiry??null,x=d?Math.max(0,Math.round((d*1e3-Date.now())/864e5)):null;return{email:p,daysToExpiry:x}})(),lastAlarm:(()=>{let l=o("last_alarm");return!l||l.state==="unknown"||l.state==="unavailable"||!l.state?null:{title:l.state,body:l.attributes?.body??"",captured_at:l.attributes?.captured_at??null,source:l.attributes?.source??null}})()}}function M(s,e){if(!s||!e)return{};let t=(o,n,a={})=>s.callService(o,n,a);return{setSetpoint:o=>t("number","set_value",{entity_id:e.entityIds.setpoint,value:o}),setSmokeLevel:o=>t("number","set_value",{entity_id:e.entityIds.smoke_level,value:o}),setProbeTarget:(o,n)=>t("number","set_value",{entity_id:e.entityIds[`probe_${o}_target`],value:n}),toggle:o=>{let n=e.entityIds[o],a=s.states[n]?.state==="on";return t("switch",a?"turn_off":"turn_on",{entity_id:n})},setText:(o,n)=>t("text","set_value",{entity_id:e.entityIds[o],value:n??""}),powerOff:()=>t("climate","set_hvac_mode",{entity_id:e.entityIds.climate,hvac_mode:"off"})}}var i={bg:"#0d1117",panel:"#161b22",panelBorder:"#30363d",text:"#e6edf3",textDim:"#8b949e",accent:"#fb923c",accentDim:"#9a3412",cool:"#3b82f6",hot:"#ef4444",steady:"#22c55e",smoke:"#a78bfa",alarm:"#f87171",stall:"#f59e0b",probe1:"#ef4444",probe2:"#06b6d4",chartChamber:"#fb923c",chartProbe1:"#ef4444",chartProbe2:"#06b6d4"},A=5;var I=`
  :host {
    display: block;
    color: ${i.text};
...
`;var B="apexcharts-card",k=class{constructor(e){this._host=e,this._mode=null,this._apexEl=null,this._canvas=null,this._series={},this._historyLoaded=!1,this._lastConfig=null}async update(e,t){let o=t.join("|");if(o!==this._lastConfig&&(this._lastConfig=o,this._historyLoaded=!1,this._series={},this._host.innerHTML="",this._mode=customElements.get(B)?"apex":"canvas",this._mode==="apex"?this._mountApex(t):this._mountCanvas()),this._mode==="apex"){this._apexEl.hass=e;return}this._historyLoaded||(this._historyLoaded=!0,await this._loadHistory(e,t)),this._appendLive(e,t),this._draw(t)}_mountApex(e){let t=e.map((n,a)=>({entity:n,name:this._labelFor(n),yaxis_id:"temp",color:[i.chartChamber,i.chartProbe1,i.chartProbe2][a]||"#fff",type:a===0?"area":"line",opacity:a===0?.25:1,stroke_width:2})),o=document.createElement(B);o.setConfig({header:{show:!1},graph_span:`${y}h`,update_interval:`${H/1e3}s`,yaxis:[{id:"temp",decimals:0,apex_config:{title:{text:"\xB0F"},forceNiceScale:!0}}],series:t}),this._apexEl=o,this._host.appendChild(o)}_mountCanvas(){this._canvas=document.createElement("canvas"),this._host.appendChild(this._canvas)}async _loadHistory(e,t){let a=`history/period/${new Date(Date.now()-y*3600*1e3).toISOString()}?filter_entity_id=${t.join(",")}&minimal_response`;try{let r=await e.callApi("GET",a);for(let c=0;c<t.length;c++){let m=t[c],h=(r?.[c]??[]).map(l=>[Date.parse(l.last_changed),parseFloat(l.state)]).filter(([l,p])=>Number.isFinite(l)&&Number.isFinite(p));this._series[m]=h}}catch(r){console.warn("[ha-prime-polaris-card] history fetch failed:",r)}}_appendLive(e,t){let o=Date.now(),n=o-y*3600*1e3;for(let a of t){let r=this._series[a]??(this._series[a]=[]),c=parseFloat(e.states[a]?.state);if(Number.isFinite(c)){let m=r.length?r[r.length-1][0]:0;o-m>=5e3&&r.push([o,c])}for(;r.length&&r[0][0]<n;)r.shift()}}_draw(e){let t=this._canvas;if(!t)return;let o=window.devicePixelRatio||1,n=t.clientWidth||600,a=240;t.width=n*o,t.height=a*o;let r=t.getContext("2d");r.scale(o,o),r.clearRect(0,0,n,a);let c=e.flatMap(u=>this._series[u]??[]);if(c.length<2){r.fillStyle=i.textDim,r.font="12px system-ui",r.fillText("collecting data\u2026",16,24);return}let m=c.map(u=>u[0]),h=c.map(u=>u[1]),l=Math.min(...m),p=Math.max(...m),d=Math.min(...h)-5,x=Math.max(...h)+5,f={l:36,r:12,t:12,b:22},w=u=>f.l+(u-l)/Math.max(1,p-l)*(n-f.l-f.r),v=u=>f.t+(1-(u-d)/Math.max(1,x-d))*(a-f.t-f.b);r.strokeStyle=i.panelBorder,r.fillStyle=i.textDim,r.font="11px system-ui",r.lineWidth=1;let P=4;for(let u=0;u<=P;u++){let g=d+u/P*(x-d),b=v(g);r.beginPath(),r.moveTo(f.l,b),r.lineTo(n-f.r,b),r.stroke(),r.fillText(g.toFixed(0),6,b+4)}let L=[i.chartChamber,i.chartProbe1,i.chartProbe2];e.forEach((u,g)=>{let b=this._series[u]??[];if(!(b.length<2)){if(r.strokeStyle=L[g]||"#fff",r.lineWidth=g===0?1.5:2,g===0){r.fillStyle=L[0]+"40",r.beginPath(),r.moveTo(w(b[0][0]),v(d));for(let[F,E]of b)r.lineTo(w(F),v(E));r.lineTo(w(b[b.length-1][0]),v(d)),r.closePath(),r.fill()}r.beginPath(),r.moveTo(w(b[0][0]),v(b[0][1]));for(let[F,E]of b)r.lineTo(w(F),v(E));r.stroke()}})}_labelFor(e){return e.includes("chamber")?"Chamber":e.includes("probe_1")?"Probe 1":e.includes("probe_2")?"Probe 2":e}};var S=-135,G=135,ne=G-S;function X(s){let e=s.size??220,t=e/2,o=e/2,n=e/2-18,a=s.min,r=s.max,c=s.current??a,m=s.target??a,h=Y(c,a,r),l=Y(m,a,r),p=s.fillColor??re(c,m),d=V(t,o,n,S,G),x=V(t,o,n,S,h),f=C(t,o,n,l);return`
  ...
  `}function Y(s,e,t){let n=(Math.max(e,Math.min(t,s))-e)/(t-e);return S+n*ne}function C(s,e,t,o){let n=(o-90)*Math.PI/180;return{x:s+t*Math.cos(n),y:e+t*Math.sin(n)}}function V(s,e,t,o,n){let a=C(s,e,t,o),r=C(s,e,t,n),c=Math.abs(n-o)>180?1:0;return`M ${a.x} ${a.y} A ${t} ${t} 0 ${c} 1 ${r.x} ${r.y}`}function re(s,e){if(s==null||e==null)return i.accent;let t=s-e;return t>A?i.hot:t<-A?i.cool:i.steady}function _(s){return String(s??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function q(s){if(!s)return"";let{chamber:e,setpoint:t,chamberDelta:o}=s,n=o!=null?`\u0394 ${o>0?"+":""}${o.toFixed(0)}\xB0F`:"",a=t??225;return`
  ...
  `}function W(s){if(!s)return"";let e=[];return s.runningStatus&&e.push({cls:"active",txt:s.runningStatus}),e.push({cls:s.activeMode==="smoke"?"smoke":"active",txt:`mode: ${s.activeMode}`}),s.smokeOn&&s.smokeLevel!=null&&e.push({cls:"smoke",txt:`smoke ${s.smokeLevel.toFixed(0)}`}),s.alarmOn&&e.push({cls:"alarm",txt:"alarm armed"}),s.winterOn&&e.push({cls:"active",txt:"winter"}),s.pushOn&&e.push({cls:"active",txt:"push on"}),s.ambient&&e.push(s.ambientResolved.value!=null?{cls:"active",txt:`ambient ${s.ambientResolved.value.toFixed(0)}\xB0F`}:{cls:"alarm",txt:"ambient unresolved"}),s.wind&&e.push(s.windResolved.value!=null?{cls:"active",txt:`wind ${s.windResolved.value.toFixed(1)}`}:{cls:"alarm",txt:"wind unresolved"}),`
    <div class="chip-row">
      ${e.map(t=>`<span class="chip ${t.cls}">${_(t.txt)}</span>`).join("")}
    </div>
  `}function U(s){if(!s)return"";let e=s.cookSession,t=e?s.protein?`${_(s.protein)}${s.weight_lb?" \xB7 "+_(s.weight_lb)+" lb":""}`:"recording":"no active cook",o=s.notes?_(s.notes):e?"":"flip the switch when you start a real cook",n=[];s.ambientResolved.value!=null&&n.push(`ambient <strong>${s.ambientResolved.value.toFixed(0)}\xB0F</strong>`),s.windResolved.value!=null&&n.push(`wind <strong>${s.windResolved.value.toFixed(1)}</strong>`);let a=n.length?`<div class="small env-line">${n.join(" \xB7 ")}</div>`:"";return`
  ...
  `}function j(s,e){if(e.temp==null)return`
      <div class="panel">
        <div class="probe">
  ...
  `:'<div class="alarm-banner dim">no alarms</div>'}function se(s){return String(s??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function te(s,e,t,o){let n="";return e&&(n=t&&t.value!=null?`<span class="resolved-badge ok">${t.value.toFixed(1)}${o}</span>`:'<span class="resolved-badge bad">unresolved</span>'),`
    <label>${s}</label>
