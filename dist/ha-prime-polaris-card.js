...
`;var j="apexcharts-card",S=class{constructor(e){this._host=e,this._mode=null,this._apexEl=null,this._canvas=null,this._series={},this._historyLoaded=!1,this._lastConfig=null}async update(e,t){let s=t.join("|");if(s!==this._lastConfig&&(this._lastConfig=s,this._historyLoaded=!1,this._series={},this._host.innerHTML="",this._mode=customElements.get(j)?"apex":"canvas",this._mode==="apex"?this._mountApex(t):this._mountCanvas()),this._mode==="apex"){this._apexEl.hass=e;return}this._historyLoaded||(this._historyLoaded=!0,await this._loadHistory(e,t)),this._appendLive(e,t),this._draw(t)}_mountApex(e){let t=e.map((o,n)=>({entity:o,name:this._labelFor(o),yaxis_id:"temp",color:[a.chartChamber,a.chartProbe1,a.chartProbe2][n]||"#fff",type:n===0?"area":"line",opacity:n===0?.25:1,stroke_width:2})),s=document.createElement(j);s.setConfig({header:{show:!1},graph_span:`${F}h`,update_interval:`${N/1e3}s`,yaxis:[{id:"temp",decimals:0,apex_config:{title:{text:"\xB0F"},forceNiceScale:!0}}],series:t}),this._apexEl=s,this._host.appendChild(s)}_mountCanvas(){this._canvas=document.createElement("canvas"),this._host.appendChild(this._canvas)}async _loadHistory(e,t){let n=`history/period/${new Date(Date.now()-F*3600*1e3).toISOString()}?filter_entity_id=${t.join(",")}&minimal_response`;try{let r=await e.callApi("GET",n);for(let c=0;c<t.length;c++){let u=t[c],m=(r?.[c]??[]).map(l=>[Date.parse(l.last_changed),parseFloat(l.state)]).filter(([l,p])=>Number.isFinite(l)&&Number.isFinite(p));this._series[u]=m}}catch(r){console.warn("[ha-prime-polaris-card] history fetch failed:",r)}}_appendLive(e,t){let s=Date.now(),o=s-F*3600*1e3;for(let n of t){let r=this._series[n]??(this._series[n]=[]),c=parseFloat(e.states[n]?.state);if(Number.isFinite(c)){let u=r.length?r[r.length-1][0]:0;s-u>=5e3&&r.push([s,c])}for(;r.length&&r[0][0]<o;)r.shift()}}_draw(e){let t=this._canvas;if(!t)return;let s=window.devicePixelRatio||1,o=t.clientWidth||600,n=240;t.width=o*s,t.height=n*s;let r=t.getContext("2d");r.scale(s,s),r.clearRect(0,0,o,n);let c=e.flatMap(h=>this._series[h]??[]);if(c.length<2){r.fillStyle=a.textDim,r.font="12px system-ui",r.fillText("collecting data\u2026",16,24);return}let u=c.map(h=>h[0]),m=c.map(h=>h[1]),l=Math.min(...u),p=Math.max(...u),d=Math.min(...m)-5,v=Math.max(...m)+5,f={l:36,r:12,t:12,b:22},x=h=>f.l+(h-l)/Math.max(1,p-l)*(o-f.l-f.r),w=h=>f.t+(1-(h-d)/Math.max(1,v-d))*(n-f.t-f.b);r.strokeStyle=a.panelBorder,r.fillStyle=a.textDim,r.font="11px system-ui",r.lineWidth=1;let z=4;for(let h=0;h<=z;h++){let $=d+h/z*(v-d),b=w($);r.beginPath(),r.moveTo(f.l,b),r.lineTo(o-f.r,b),r.stroke(),r.fillText($.toFixed(0),6,b+4)}let I=[a.chartChamber,a.chartProbe1,a.chartProbe2];e.forEach((h,$)=>{let b=this._series[h]??[];if(!(b.length<2)){if(r.strokeStyle=I[$]||"#fff",r.lineWidth=$===0?1.5:2,$===0){r.fillStyle=I[0]+"40",r.beginPath(),r.moveTo(x(b[0][0]),w(d));for(let[A,C]of b)r.lineTo(x(A),w(C));r.lineTo(x(b[b.length-1][0]),w(d)),r.closePath(),r.fill()}r.beginPath(),r.moveTo(x(b[0][0]),w(b[0][1]));for(let[A,C]of b)r.lineTo(x(A),w(C));r.stroke()}})}_labelFor(e){return e.includes("chamber")?"Chamber":e.includes("probe_1")?"Probe 1":e.includes("probe_2")?"Probe 2":e}};var R=-135,V=135,de=V-R;function Y(i){let e=i.size??220,t=e/2,s=e/2,o=e/2-18,n=i.min,r=i.max,c=i.current??n,u=i.target??n,m=q(c,n,r),l=q(u,n,r),p=i.fillColor??ue(c,u),d=U(t,s,o,R,V),v=U(t,s,o,R,m),f=E(t,s,o-10,l),x=E(t,s,o+10,l);return`
  ...
  `}function q(i,e,t){let o=(Math.max(e,Math.min(t,i))-e)/(t-e);return R+o*de}function E(i,e,t,s){let o=(s-90)*Math.PI/180;return{x:i+t*Math.cos(o),y:e+t*Math.sin(o)}}function U(i,e,t,s,o){let n=E(i,e,t,s),r=E(i,e,t,o),c=Math.abs(o-s)>180?1:0;return`M ${n.x} ${n.y} A ${t} ${t} 0 ${c} 1 ${r.x} ${r.y}`}function ue(i,e){if(i==null||e==null)return a.accent;let t=i-e;return t>P?a.hot:t<-P?a.cool:a.steady}var G=[{id:"brisket",name:"Brisket Low & Slow",description:"225\xB0F \xB7 P1\u2192203\xB0F",apply:{setpoint:225,smoke_mode:!1,probe_1_target:203,protein:"brisket"}},{id:"brisket_smoke",name:"Brisket + Smoke",description:"225\xB0F \xB7 smoke 5 \xB7 P1\u2192203\xB0F",apply:{setpoint:225,smoke_mode:!0,smoke_level:5,probe_1_target:203,protein:"brisket"}},{id:"pork_shoulder",name:"Pork Shoulder",description:"250\xB0F \xB7 P1\u2192203\xB0F",apply:{setpoint:250,smoke_mode:!1,probe_1_target:203,protein:"pork shoulder"}},{id:"ribs",name:"Ribs",description:"225\xB0F \xB7 smoke 4 \xB7 P1\u2192195\xB0F",apply:{setpoint:225,smoke_mode:!0,smoke_level:4,probe_1_target:195,protein:"ribs"}},{id:"chicken",name:"Chicken",description:"375\xB0F \xB7 P1\u2192165\xB0F",apply:{setpoint:375,smoke_mode:!1,probe_1_target:165,protein:"chicken"}},{id:"burgers",name:"Burgers",description:"400\xB0F \xB7 P1\u2192160\xB0F",apply:{setpoint:400,smoke_mode:!1,probe_1_target:160,protein:"burgers"}},{id:"sear",name:"Sear",description:"500\xB0F",apply:{setpoint:500,smoke_mode:!1}},{id:"smoke_only",name:"Smoke Only",description:"200\xB0F \xB7 smoke 8",apply:{setpoint:200,smoke_mode:!0,smoke_level:8}}];function T(i){if(!Array.isArray(i)||i.length===0)return G;let e=new Map(G.map(t=>[t.id,t]));for(let t of i)!t||!t.id||e.set(t.id,{id:t.id,name:t.name||t.id,description:t.description||"",apply:t.apply||{}});return Array.from(e.values())}function _(i){return String(i??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function Z(i){if(!i)return"";let{chamber:e,setpoint:t,chamberDelta:s}=i,o=s!=null?`\u0394 ${s>0?"+":""}${s.toFixed(0)}\xB0F`:"",n=t??225;return`
  ...
  `}function X(i){return i?String(i).split(/(\s+)/).map(e=>e.length===0||/\s+/.test(e)?e:e[0].toUpperCase()+e.slice(1).toLowerCase()).join(""):""}function K(i){if(!i)return"";let e=i.cookSession,t=e?i.protein?`${_(i.protein)}${i.weight_lb?" \xB7 "+_(i.weight_lb)+" lb":""}`:"recording":"no active cook",s=i.notes?_(i.notes):e?"":"flip the switch when you start a real cook",o=[];i.ambientResolved.value!=null&&o.push(`<span class="chip active">Ambient ${i.ambientResolved.value.toFixed(0)}\xB0F</span>`),i.windResolved.value!=null&&o.push(`<span class="chip active">Wind ${i.windResolved.value.toFixed(1)}</span>`);let n=o.length?`<div class="chip-row env-chips">${o.join("")}</div>`:"";return`
